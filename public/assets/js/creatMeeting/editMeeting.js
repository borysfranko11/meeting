    ...
    created (){
        ...
        $.ajax({
            ...
            success: function (res) {
                // console.log(res)
                if(res.Success == true){
                    resDataBasic = res.Data.base[0] || '';
                    resDataBudget = res.Data.budget || '';
                    resDataPeople = res.Data.humans || '';
                    dot = res.Data.dot;
                    that.basicInfor = resDataBasic;
                    that.meetingCode = resDataBasic.meeting_code;
                    that.budget = resDataBudget;
                    that.peop = resDataPeople;
                    var resCostCenter = resDataBasic.marketorgcode;
                    var resProvice = resDataBasic.provincedesc;
                    var resProviceCode = resDataBasic.provincecode;
                    // console.log(typeof resProviceCode)
                    var resCity = resDataBasic.citydesc;
                    var resTripStartTime = resDataBasic.trip_start_time;
                    var resTripEndTime = resDataBasic.trip_end_time;
                    var resStartTime = resDataBasic.start_time;
                    var resEndTime = resDataBasic.end_time;
                    var resTimeArr = resDataBasic.time_extend;
                    // 参会人员
                    var resClientNum = resDataBasic.clientele_num;
                    var resWithinNum = resDataBasic.within_num;
                    var resThirdNum = resDataBasic.nonparty_num;
                    // 会议类型
                    var resMeetinType = resDataBasic.meeting_type_code;
                     bus.$emit("eventMeetinType", resMeetinType);
                     // 竞标类型
                    var resMeetingBitType = resDataBasic.bit_type;
                     bus.$emit("eventMeetingBitType", resMeetingBitType);
                    // 申请部门
                    var resApply = resDataBasic.department;
                     bus.$emit("eventApply", resApply);
                    // 成本中心
                    var resCost = resDataBasic.marketorgcode;
                      // 预算是否对服务商可见
                    var resVisi = resDataBasic.look_budget;
                     bus.$emit("eventVisi", resVisi);
                    // 分发器；
                   
                    // basicInfor
                    bus.$emit("eventMeetingName", resDataBasic);
                        // 成本中心；
                        bus.$emit("eventCostCenter", resCostCenter);
                        // 会议地点省份；
                        bus.$emit("eventProvice", resProvice);
                        // 会议地点城市；
                        bus.$emit("eventCity", resCity);
                        // 行程开始时间；
                        bus.$emit("eventTripStartTime", resTripStartTime);
                        // 行程结束时间；
                        bus.$emit("eventTripEndTime", resTripEndTime);
                        // 会议开始时间；
                        // bus.$emit("eventStartTime", resStartTime);
                         // 会议结束时间；
                        // bus.$emit("eventEndTime", resEndTime);
                         // 会议数组时间（大于一个时，以数组形式展示）；
                        bus.$emit("eventTimeArr", resTimeArr);
                        
                    // basicBudget
                    bus.$emit("eventBudget", resDataBudget)
                    // People
                    bus.$emit("eventClient", resClientNum)
                    bus.$emit("eventWithin", resWithinNum)
                    bus.$emit("eventThird", resThirdNum)
                    bus.$emit("eventPeople", resDataPeople)
                    // bus.$emit("eventPro", pro)
                    if(dot){
                        var htmlDot = '';
                        $.each(dot, function (n, value) {
                            //alert(n + ' ' + value['rfp_id']);
                            if(n == 0){
                                htmlDot += "<div class='mb15 el-row'><div class='block el-col el-col-12'><div class='el-date-editor el-input el-date-editor--date' lazy='' id='' style='width: 97%;'><input autocomplete='off' placeholder='请输入签到点' value='"+value['dot_name']+"' size='small' type='text' rows='2' validateevent='true' class='el-input__inner' name='dot'></div></div></div>";
                            }else{
                                htmlDot += "<div class='mb15 el-row'><div class='block el-col el-col-12'><div class='el-date-editor el-input el-date-editor--date' lazy='' id='' style='width: 97%;'><input autocomplete='off' placeholder='请输入签到点' value='"+value['dot_name']+"' size='small' type='text' rows='2' validateevent='true' class='el-input__inner' name='dot'></div></div><div align='center' class='el-col-4'><button type='button' class='el-button el-button--danger' onclick='remove(this)'>删除</button></div></div>";
                            }
...
var oCorPar = Vue.extend(oparent)
var contParent = new oCorPar().$mount('#parent');
...
var Main2 = {
    ...
    created (){
        var that = this;
        bus.$on("eventMeetingName", function (resDataBasic) {
            if(resDataBasic.abroad_file){
                // console.log(resDataBasic.abroad_file);
                that.imageUrl = resDataBasic.abroad_file;
                var obj = {};
                obj.name = "已上传文件";
                obj.url = resDataBasic.abroad_file;
                that.fileList3.push(obj);
            }       
        })
    },
    methods: {
...
var Main3 = {
    ...
    created(){
        var that = this;
        // bus.$on("eventStartTime", function (resStartTime) {
        //     if(resStartTime){
        //         var time1 = new Date(resStartTime)
        //         that.items[0].value1 = time1;
        //     }
        // });
        // bus.$on("eventEndTime", function (resEndTime) {
        //     if(resEndTime){
        //         var time2 = new Date(resEndTime)
        //         that.items[0].value2 = time2;
        //     }
        // });
        bus.$on("eventTimeArr", function (resTimeArr) {
            if(resTimeArr && resTimeArr.length >0){
                console.log(resTimeArr)
                var timeArr = [];
                timeArr.length = resTimeArr.length;
                for (var i = 0; i < resTimeArr.length; i++) {
                    timeArr[i] = {};
                    timeArr[i].value1 = '';
                    timeArr[i].value2 = '';
                    timeArr[i].value3 = ''; 
                ...
                for (var i = 0; i < resTimeArr.length; i++) {
                    timeArr[i].value1 = new Date(resTimeArr[i].start_time);
                    timeArr[i].value2 = new Date(resTimeArr[i].end_time);
                    timeArr[i].value3 = ''; 
                    timeArr[i].value4 = ''; 
                }
                that.items = timeArr;
                // var time2 = new Date(resTimeArr)
                // that.items[0].value2 = time2;
            }
        });
    },
...
var Main4 = {
    ...
    created(){
        var that = this;
        bus.$on("eventTripStartTime", function (resTripStartTime) {
            if(resTripStartTime){
                var time = new Date(resTripStartTime)
                // console.log(time)
                that.value1 = time;
            }
        });
        bus.$on("eventTripEndTime", function (resTripEndTime) {
            if(resTripEndTime){
                var time2 = new Date(resTripEndTime)
                that.value2 = time2;
            }
        })
    },
...
var Main5 = {
    ...
    created (){
        var that = this;
        // 获取城市列表；
        bus.$on("eventCity", function (resCity) {
            if(resCity){
                that.cityText = resCity;
            }
        })
        bus.$on("eventProvice", function (resProvice) {
            if(resProvice){
                // that.proChange(resProvice);
                that.proText = resProvice;
               // that.proText = that.options[resProvice]["name"];
            }
        });
        $.ajax({
            ...
            success: function (res) {
                ...
                if(res.Success == true && res.Data != 0){
                    ...
                    for(var key in res.Data){
                        ...
                        that.options.push(res.Data[key])
                    }
                    that.setPro();
    ...
    methods: {
        ...
        setPro (){
            ...
            // that.proChange (that.valuePro)
        },
        proChange (proText){
            // console.log(proText)
            var obj = {};
            obj = this.options.find(function (item) {
                return item.name === proText;
            });
            this.valuePro = obj.id;
            proId = obj.id
            if(this.num == 0){
                this.num = 1;
            }else{
            ...
            $.ajax({
                ...
                success: function (res) {
                    if(res.Success == true && res.Data != 0){
                        ...
                        that.options2 = [];
                        for(var key in res.Data){
                            that.options2.push(res.Data[key])
                        }
                        if(that.num2 == 0){
        ...
        cityChange (cityText){
            ...
            if(!this.options2 || !cityText || this.options2.length == 0) return;
            var obj1 = {};
            
            // setTimeout(function (argument) {
            obj1 = this.options2.find(function (item) {
                return item.name === cityText;
            })
            // console.log(obj1)
            this.valueCity = obj1.id;
            // },10)
            
            // this.cityText = this.options2[valueCity]["name"];
            // this.proText = valueCity;
        }
    }
  };
...
var Main6 = {
    ...
    created: function () {
        var that = this;
        bus.$on("eventMeetinType", function (resMeetinType) {
            if (resMeetinType){
                that.value8 = resMeetinType;
            }
        })
        $.ajax({
            url:'/Meeting/getMeetingType',
    ...
    beforeDestroy (){
        bus.$off("eventMeetinType",resMeetinType);
    }
};
...
var Main14 = {
    ...
    created: function () {
        var that = this;
        bus.$on("eventMeetingBitType", function (resMeetingBitType) {
            if (resMeetingBitType !== ''){
                that.value14 = resMeetingBitType;
            }
        })
        $.ajax({
    ...
    beforeDestroy (){
        bus.$off("eventMeetingBitType",resMeetingBitType);
    }
};
...
var Main7 = {
    ...
    created () {
        var that = this;
        bus.$on("eventVisi", function (resVisi) {
            if(resVisi){
                if(resVisi == 1){
                    that.state = true;
                }else{
        ...
        $.ajax({
            ...
            success: function (res) {
                // console.log(res);
                that.items = res;
                bus.$on("eventBudget", function (resDataBudget) {
                    if(resDataBudget){
                        for (var i = 0; i < resDataBudget.length; i++) {
                            for (var j = 0; j < that.items.length; j++) {
                                if(resDataBudget[i].fundtypecode == that.items[j].id){
                                    that.items[j].num = Number(resDataBudget[i].budgetamount);
                                    that.items[j].checked = true;
                                }
                            }
                        }
                        // that.items = resDataBudget;
                    }
                });
            },
            error: function (res) {
...
var Main8 = {
    ...
    created: function () {
        var that = this;
        bus.$on("eventApply", function (resApply) {
            if(resApply){
                // console.log(resApply)
                that.value8 = resApply;
            }
        })
        $.ajax({
    ...
    methods: {
        changeValue (value8){
            // console.log(value8);
            if(!this.optiongs || !value8) return;
            var obj = {};
            obj = this.options.find(function (item) {
                return item.id === value8;
            });
...
var Main9 = {
    ...
    created: function () {
        var that = this;
        bus.$on("eventCostCenter", function (resCostCenter) {
            // console.log(resDataBasic.marketorgcode)
            if(resCostCenter){
                // console.log(typeof resCostCenter)
                that.value8 = resCostCenter;
            }
        })
        $.ajax({
...
var Main11 = {
    ...
    created(){
        var that = this;
        bus.$on("eventClient", function (resClientNum) {
            if(resClientNum){
                // console.log(resClientNum);
                that.num1 = resClientNum;
            }
        })
        bus.$on("eventWithin", function (resWithinNum) {
            if(resWithinNum){
                // console.log(resWithinNum);
                that.num2 = resWithinNum;
            }
        })
        bus.$on("eventThird", function (resThirdNum) {
            if(resThirdNum){
                // console.log(resThirdNum);
                that.num3 = resThirdNum;
            }
        })
    },
    methods: {
...
var oMain = {
    ...
    created (){
        var that = this;
        bus.$on("eventMeetingName", function (resDataBasic) {
            that.oval = resDataBasic.meeting_name;
        })
        // this.oval = resDataBasic.meeting_name;
    }
}
var oCtor = Vue.extend(oMain)
var omeetingName2 = new oCtor().$mount('#meetingName2');

function assignText(res) {
    resDataBasic = res.Data.base[0];
    console.log(resDataBasic)
    omeetingName2.val = resDataBasic.meeting_name
}
//会议托管提交的逻辑；
...
$(".actions ul li").on('click', function () {
   ...
   if( oindex==2 || oindex == 3){
    ...
    //基础信息模块
    var meetingCode = contParent.meetingCode;
    ...
    /*if(oindex == 2){  
        if(!numberCustomer || numberCustomer <= 0){
            layFun("客户参会人数填写不正确");
            return false;
        }
    ...
    var data = {
        rfp_id : id,         //会议id--------------------直接提交可以为空，草稿编辑时不能为空
