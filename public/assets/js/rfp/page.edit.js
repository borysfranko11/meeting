...
$( document ).ready( function()
{
    ...
    var star_types = {"5": '五星',"4": '四星',"3": '三星',"2": '其他'};
   ...
   /* -=== 多节点创建和插件绑定配置 START ===- */
    ...
    var datapicker_options = {
        language: 'zh',
        minDate: '',
        //minDate: new Date(year, month, date),                 // 禁止选择今天之前的日期
    ...
    // 待批量初始化配置
    var init_list = [
        {"addr":"wizard-p-2","index":"place","reg":/place_duplicate_layout_(.*)/},
        {"addr":"wizard-p-3","index":"food","reg":/food_duplicate_layout_(.*)/},
        {"addr":"wizard-p-3","index":"wine","reg":/wine_duplicate_layout_(.*)/},
        {"addr":"wizard-p-4","index":"room","reg":/room_duplicate_layout_(.*)/}
    ];

    $.each( init_list, function( key, content )
    {
        // 插件自动批量初始化
        $( '#'+content["addr"] ).find( 'div[id^='+content["index"]+'_duplicate_layout_]' ).each( function()
        {
            var id = $( this ).attr( 'id' );
            var count = id.match( content["reg"] )[1];

            // 屏蔽模板存储空间
            if( id.indexOf( '{key}' ) < 0 )
            {
                batchInitPlugins( plugin_store[content["index"]], '#'+id, count );
            }
        } );
    } );
    ...
    function batchInitPlugins( obj_store, addr, index )
    {
        ...
        $( addr ).find( 'input' ).each( function()
        {
            ...
            if( typeof obj_store[index][node_index]["value"] === 'undefined' )
            {
                obj_store[index][node_index]["value"] = {};
            }
            ...
            switch( plugin )
            {
                case 'start-time':
                    ...
                    if( date_cache )
                    {
                        date_cache = formatDatepicker( Date.parse( new Date( date_cache ) ) );
                        obj_store[index][node_index]["init"].data( 'datepicker' ).selectDate( new Date( date_cache.getFullYear(), date_cache.getMonth(), date_cache.getDate(), date_cache.getHours(), date_cache.getMinutes() ) );
                    }
                    break;
                case 'end-time':
                    if( $( this ).val().length > 1 )
