...
$( document ).ready( function()
{
    ...
    function GetRequest() {
        ...
        var url = '?city_id='+CITY_ID+'&position='+POSITION+'&place_type='+PLACE_TYPE+'&place_star='+PLACE_STAR+'&key_words='; //获取url中"?"符后的字串
    ...
    function genCommendList( data )
    {
        ...
        for( var key in rows )
        {
            ...
            var _place_id = rows[key]["place_id"],                                                                                      // 推荐列表索引
                ...
                _area = !_.isUndefined( rows[key]["meeting_basic"]["max_area"] ) ? rows[key]["meeting_basic"]["max_area"] : ' ',                                                  // 会议室最大面积
                _roomCount = !_.isUndefined( rows[key]["rooms"][0]['count'] ) ? rows[key]["rooms"][0]['count'] : ' ',                                   // 客房数量
                _meetingRoomCount = !_.isUndefined( rows[key]["meeting_basic"]['count'] ) ? rows[key]["meeting_basic"]['count'] : ' ',              // 会议室数量
                _maxCapacity = !_.isUndefined( rows[key]["meeting_basic"]['max_capacity'] ) ? rows[key]["meeting_basic"]['max_capacity'] : ' ',                             // 最多可纳人数
                ...
                _main_pic_url = rows[key]["main_pic"] != 'http://pic.eventown.com'  ? rows[key]["main_pic"] : (!_.isUndefined(rows[key]["pics"][0]) ? rows[key]["pics"][0]['pic_url'] : ''),                 // 图片地址
